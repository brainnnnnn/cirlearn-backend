...
export function sanitizeForStreaming(html: string): string {
  let out = html
    ...
    .replace(/<script[\s\S]*?<\/script>/gi, '') // remove complete script blocks
    .replace(
  ...
  // Any remaining <script> must be unclosed (partial stream) — truncate before it
  // to prevent raw JS content from rendering as visible text
  const unclosed = out.search(/<script\b/i);
